"""Candlecam adapter for WebThings Gateway."""
...
class CandlecamAdapter(Adapter):
    ...
    def __init__(self, verbose=False):
        ...
        try:
            ...
            print("Could not get actual own IP address: " + str(ex))
        ...
        self.radio_stations = []
        self.radio_stations_names_list = ['FIP']
        ...
        try:
            candlecam_device = CandlecamDevice(self, self.radio_stations_names_list, self.audio_output_options)
            self.handle_device_added(candlecam_device)
            if self.DEBUG:
...
#
    ...
    def set_status_on_thing(self, status_string):
        ...
        try:
            if self.devices['candlecam'] != None:
                #self.devices['candlecam'].properties['status'].set_cached_value_and_notify( str(status_string) )
                self.devices['candlecam'].properties['status'].update( str(status_string) )
        except Exception as ex:
            print("Error setting status of candlecam device: " + str(ex))
    ...
    def set_state_on_thing(self, power):
        ...
        try:
            if self.devices['candlecam'] != None:
                self.devices['candlecam'].properties['power'].update( bool(power) )
        except Exception as ex:
            print("Error setting power state of candlecam device:" + str(ex))
    ...
    def set_station_on_thing(self, station):
        ...
        try:
            if self.devices['candlecam'] != None:
                self.devices['candlecam'].properties['station'].update( str(station) )
        except Exception as ex:
            print("Error setting station of candlecam device:" + str(ex))
    ...
    def set_volume_on_thing(self, volume):
        ...
        try:
            if self.devices['candlecam'] != None:
                self.devices['candlecam'].properties['volume'].update( int(volume) )
        except Exception as ex:
            print("Error setting volume of candlecam device:" + str(ex))
            ...
            for option in self.audio_controls:
                if str(option['human_device_name']) == str(selection):
                    ...
                    try:
                        ...
                        if self.devices['candlecam'] != None:
                            self.devices['candlecam'].properties['audio output'].update( str(selection) )
                    except Exception as ex:
    ...
    def unload(self):
        if self.DEBUG:
            print("Shutting down Candlecam.")
        self.set_status_on_thing("Bye")
        self.set_radio_state(0)
        self.running = False
        candlecam_adapter_running = False
...
class CandlecamDevice(Device):
    ...
    def __init__(self, adapter, radio_station_names_list, audio_output_list):
        ...
        try:
            ...
            try:
                #mjpg_stream_url = 'http://' + self.adapter.own_ip + ':8000/media/stream.mjpg'
                dash_stream_url = 'http://' + self.adapter.own_ip + '/extensions/candlecam/stream/index.mpd'
                m3u8_stream_url = 'http://' + self.adapter.own_ip + '/extensions/candlecam/stream/index.mpd'
                #dash_stream_url = '/media/candlecam/stream/index.mpd'
                print("dash_stream url: " + str(dash_stream_url))
                
                self.properties['stream'] = CandlecamProperty(
                                self,
                                'stream',
                                {
                                    'name': 'stream',
                                    ...
                                    'links':[
                                        {
                                            ...
                                            'href':'/media/' + self.adapter.addon_name + '/photos/latest.jpg',
                ...
                print("error adding camera properties: " + str(ex))
            
            
            self.properties["station"] = CandlecamProperty(
                            self,
                            "station",
                            {
                                'label': "Station",
            ...
            self.properties["status"] = CandlecamProperty(
                            self,
                            "status",
                            {
                                'label': "Status",
            ...
            self.properties["power"] = CandlecamProperty(
                            self,
                            "power",
                            {
                                '@type': 'OnOffProperty',
            ...
            self.properties["volume"] = CandlecamProperty(
                            self,
                            "volume",
                            {
                                '@type': 'LevelProperty',
